...
/* SingleFile stores a picture the page uses MORE THAN ONCE in a CSS variable (--sf-img-N: url("data:…")) and leaves the <img> holding an
   ...
   back into src, so every reader downstream sees an ordinary image. (Found in hs2-test3, 21 Sep 2026.) */
...
export function dataImgFile(src) {
  ...
  /* a single-file save leaves a PLACEHOLDER where an image never loaded (an inline non-base64 SVG, or data:,). A placeholder is not
     her figure. Real figures are base64 and carry real bytes. */
